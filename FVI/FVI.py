...
def train_dqn_instance(x_dim, y_dim, time_horizon, samplepath_x, samplepath_y,
    ...
    for t in range(time_horizon, -1, -1):
        ...
        for smp_id in range(smp_size):
            ...
            if t == 0:
                indices_x = np.random.choice(samplepath_x.shape[0], size=in_sample_size, replace=False)
                indices_y = np.random.choice(samplepath_x.shape[0], size=in_sample_size, replace=False)
                samples_x = samplepath_x[indices_x, t+1].reshape(-1, 1)  # adjust reshape if x_dim > 1
                samples_y = samplepath_y[indices_y, t+1].reshape(-1, 1)  # adjust reshape if y_dim > 1
...
# Use NN from the knn approach for CDE
def train_dqn_instance_mult(x_dim, y_dim, time_horizon, samplepath_x, samplepath_y,
                       n_opt, in_sample_size,
                       device, discount=1, mem_size=3000, trunc_flag=True, n_iter = 1500):
    """
    Trains a single instance of a DQN for conditional density estimation.
    ...
    for t in range(time_horizon, -1, -1):
        if t < time_horizon and t> 0:
            ...
            data_tensor = torch.tensor(data, dtype=torch.float32).to(device)
            x_estimator, x_loss_hist, x_n_nan = train_conditional_density(data_tensor, d_X=d_X, d_Y=d_Y, k=in_sample_size,
                                                             n_iter=n_iter, n_batch=50, lr=1e-3, nns_type=' ', Lip = True, Print_res = True)
        ...
        for smp_id in range(smp_size):
            ...
            if t < time_horizon and t>0:
                ...
                with torch.no_grad():
                    x0_tensor = torch.tensor([[samplepath_x[smp_id,1:t+1]]], dtype=torch.float32, device=device)
                    y0_tensor = torch.tensor([[samplepath_y[smp_id, 1:t+1]]], dtype=torch.float32, device=device)
                    x_est = x_estimator.atomnet(x0_tensor)  
                    y_est = y_estimator.atomnet(y0_tensor) 
