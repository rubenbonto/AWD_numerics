...
def gurobi_bm(margs, f, p_dist=2, radial_cost=0, f_id=0, minmax='min', r_opti=0, outputflag=1, causal=0, anticausal=0):
    ...
    # Compute cost matrix
    if radial_cost == 0:
        cost_mat = np.array([[f(xl_1[i, :], xl_2[j, :]) for j in range(n2)] for i in range(n1)])
    else:
            ...
            for ind_t in range(len(x_t_arr)):
                ...
                for ind_t_y in range(len(y_t_arr)):
                    ...
                    for ind_xp in range(len(x_tp_arr)):
                        ...
                        pi_sum_left = quicksum(pi_var[i_x, i_y] for i_x in pos_xtp_real for i_y in pos_h_y)
                        pi_sum_right = quicksum(pi_var[i_x, i_y] for i_x in pos_h for i_y in pos_h_y)
                        mu_sum_left = sum(pl_1[i_x] for i_x in pos_h)
                        mu_sum_right = sum(pl_1[i_x] for i_x in pos_xtp_real)
                        m.addConstr(pi_sum_left * mu_sum_left == pi_sum_right * mu_sum_right)
        ...
        for t in range(1, n_dim):
            ...
            for ind_t in range(len(x_t_arr)):
                ...
                for ind_t_y in range(len(y_t_arr)):
                    ...
                    for ind_xp in range(len(x_tp_arr)):
                        pos_xtp_real = pos_h[np.where(ind_inv_p == ind_xp)[0]]
                        pi_sum_left = quicksum(pi_var[i_y, i_x] for i_x in pos_xtp_real for i_y in pos_h_y)
                        pi_sum_right = quicksum(pi_var[i_y, i_x] for i_x in pos_h for i_y in pos_h_y)
