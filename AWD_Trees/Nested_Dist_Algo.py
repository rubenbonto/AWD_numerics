...
def nested_optimal_transport_loop(tree1_root, tree2_root, max_depth, method, lambda_reg, power):
    ...
    elif method not in ("solver_lp", "solver_pot", "Sinkhorn", "solver_sinkhorn", "solver_lp_pot","solver_pot_sinkhorn", "solver_jax"):
        raise ValueError("Method must be one of 'Sinkhorn', 'solver_lp', 'solver_jax', 'solver_lp_pot', 'solver_pot_sinkhorn' or 'solver_pot'.")
    ...
    for depth in range(max_depth - 1, -1, -1):
        ...
        for i, path1 in enumerate(paths_tree1):
            for j, path2 in enumerate(paths_tree2):
                ...
                start_row, end_row = sum(children_count_tree1[:i]), sum(children_count_tree1[:i+1])
                start_col, end_col = sum(children_count_tree2[:j]), sum(children_count_tree2[:j+1])
                sub_matrix = full_distance_matrix[start_row:end_row, start_col:end_col]
                
                pi_ratios, pi_tilde_ratios = compute_marginal_probabilities_for_subset(path1, path2, tree1_root, tree2_root)
                ...
                if method == "Sinkhorn":
                    ...
                    probability_matrix = solver_pot_sinkhorn(sub_matrix, pi_ratios, pi_tilde_ratios, epsilon=(1/lambda_reg))
                elif method == "solver_pot_1D":
                    probability_matrix = solver_pot_1D(sub_matrix, pi_ratios, pi_tilde_ratios)
                else:
