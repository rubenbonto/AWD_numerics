import numpy as np
from scipy.stats import logistic  # or use norm for Gaussian kernel
        ...
        for t in range(T):
            ...
            if t < T - 1:
                if Markovian:
                    ...
                    w = np.array([kernelDistribution(loc=data[j, t], scale=ht).pdf(x[t]) for j in range(N)])
                else:
                    # For non-Markovian (scenario trees), multiply the weights.
                    w = w * np.array([kernelDistribution(loc=data[j, t], scale=ht).pdf(x[t]) for j in range(N)])
        return x
    return closure
...
def train_conditional_density_kernel(data, kernelDistribution=logistic, Markovian=True):
    ...
    estimator = kernel_scenarios(data_float, kernelDistribution=kernelDistribution, Markovian=Markovian)
    return estimator
