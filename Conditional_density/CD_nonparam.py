...
def estimate_conditional_density_one_step(data, x0, bandwidth_joint, bandwidth_marginal, y_grid, kernel='logistic'):
    """
    ...
    if kernel == 'gaussian':
        ...
        f_x = np.sum(logistic_kernel((x0 - data[:, 0]) / bandwidth_marginal)) / (n * bandwidth_marginal)
...
def estimate_conditional_density_two_step(data, x0, bandwidth_x, bandwidth_e, y_grid, kernel='logistic'):
    """
